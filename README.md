...
- **Database Explorer** — a generic, read-only browser over the actual database tables (twelve of them, one click away in the sidebar), sortable by any column. Every other screen in the app hand-curates its columns and goes through the service layer; this one binds straight to each repository's raw `GetAllAsync()` and lets `DataGridView` build columns via reflection, because the point here is showing the real schema, not another polished business view of it
- **CSV Export** — every list screen (and the Database Explorer) has an Export CSV button that writes out exactly what's on screen — current search/filter/sort included — since it reads the grid's already-formatted cells rather than re-querying
