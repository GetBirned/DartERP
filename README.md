...
- **Purchase Orders** — multi-line orders with a product picker, automatic line and order total calculation, Draft → Submitted → Approved → Received/Cancelled status workflow, validation (active vendor required, at least one line before submission, non-negative quantities/costs), and a status history panel logging who changed the status and when
...
- **Database Explorer** — a generic, read-only browser over the actual database tables (eleven of them, one click away in the sidebar), sortable by any column. Every other screen in the app hand-curates its columns and goes through the service layer; this one binds straight to each repository's raw `GetAllAsync()` and lets `DataGridView` build columns via reflection, because the point here is showing the real schema, not another polished business view of it
