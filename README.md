...
- **Purchase Orders** — multi-line orders with a product picker, automatic line and order total calculation, Draft → Submitted → Approved → Received/Cancelled status workflow, validation (active vendor required, at least one line before submission, non-negative quantities/costs), a status history panel logging who changed the status and when, and file attachments (invoices, packing slips) with open/remove actions
- **Work Orders** — production orders against a product with Planned → Released → In Production → Quality Control → Completed/Cancelled status and the same status history panel as Purchase Orders; completed or cancelled orders are locked against further edits
...
- **Database Explorer** — a generic, read-only browser over the actual database tables (twelve of them, one click away in the sidebar), sortable by any column. Every other screen in the app hand-curates its columns and goes through the service layer; this one binds straight to each repository's raw `GetAllAsync()` and lets Syncfusion's `SfDataGrid` build columns via reflection, because the point here is showing the real schema, not another polished business view of it
- **Grids & Charts** — every grid in the app (all eleven of them) and both dashboard charts run on Syncfusion's WinForms suite (`SfDataGrid`, `ChartControl`), styled by hand from this app's own tan-and-black theme rather than a prebuilt Syncfusion theme
- **CSV Export** — every list screen (and the Database Explorer) has an Export CSV button that writes out exactly what's on screen — current search/filter/sort included — reading straight from each row's bound data rather than re-querying
- **Tech Glossary** — an in-app, first-person write-up of every technology used and why, paired with two hand-drawn GDI+ diagrams (the layer dependency graph and a request's actual path from the UI down to SQL Server) — the reasoning behind the build lives in the app itself, not just this README
