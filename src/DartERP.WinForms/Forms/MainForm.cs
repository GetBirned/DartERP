...
public class MainForm : Form
{
    ...
    private Dictionary<string, Func<Control>> BuildModuleFactories() => new()
    {
        ...
        ["Inventory"] = () => new InventoryControl(_serviceProvider.Resolve<InventoryService>()),
        ["Purchase Orders"] = () => new PurchaseOrderListControl(
            _serviceProvider.Resolve<PurchaseOrderService>(),
            _serviceProvider.Resolve<VendorService>(),
            _serviceProvider.Resolve<ProductService>()),
        ["Work Orders"] = () => new WorkOrderListControl(
            _serviceProvider.Resolve<WorkOrderService>(),
            _serviceProvider.Resolve<ProductService>()),
        ["Serialized Inventory"] = () => new SerializedItemListControl(
            _serviceProvider.Resolve<SerializedItemService>(),
            _serviceProvider.Resolve<WorkOrderService>()),
        ["Quality Control"] = () => new QualityControlListControl(
            _serviceProvider.Resolve<QualityInspectionService>(),
            _serviceProvider.Resolve<SerializedItemService>()),
        ["A&D Log"] = () => new DispositionListControl(
            _serviceProvider.Resolve<DispositionService>(),
            _serviceProvider.Resolve<SerializedItemService>(),
            _serviceProvider.Resolve<CustomerService>()),
        ["Database"] = () => new DatabaseExplorerControl(
            _serviceProvider.Resolve<ICustomerRepository>(),
            _serviceProvider.Resolve<IVendorRepository>(),
            _serviceProvider.Resolve<IProductRepository>(),
            _serviceProvider.Resolve<IPurchaseOrderRepository>(),
            _serviceProvider.Resolve<IWorkOrderRepository>(),
            _serviceProvider.Resolve<ISerializedItemRepository>(),
            _serviceProvider.Resolve<IQualityInspectionRepository>(),
            _serviceProvider.Resolve<IDispositionRepository>(),
            _serviceProvider.Resolve<IUserRepository>()),
